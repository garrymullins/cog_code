...
public class ViewPackages
{
	...
	* @return	Returns a string that either shows the name of each package in the content store 
	*			or displays a message to indicate that no packages have been published.
	*/
	...
	public String viewPackages(CRNConnect connection)
	{
		...
		if (connection.getCMService() != null)
		{
			...
			// If packages exist in the content store, the output shows the package name
			// on one line, followed by a second line that shows the search path of the package.
	...
	public BaseClass[] getPackages(CRNConnect connection)
	{
		...
		if (connection.getCMService() != null)
		{
			...
			try
			{
				...
				*        	Specifies the search path string so that Content Manager can locate the requested
				*			objects, which are packages in this example.
				* @param 	props
				* 			Specifies alternate properties that you want returned for the package object. 
				*			When no properties are specified, as in this example, the default properties of
