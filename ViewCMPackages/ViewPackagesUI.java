...
// display a window.
public class ViewPackagesUI extends JFrame
{
	...
	private JButton viewPkgsButton;

	private static Logon sessionLogon;

	// This is the constructor.
	public ViewPackagesUI(String title, CRNConnect connection)
	{
	...
	private void addComponents()
	{
		...
		viewPkgsButton = new JButton("View Package Info");
		viewPkgsButton.addActionListener(new allButtonsHandler());
		JPanel buttonPanel = new JPanel();
		buttonPanel.add(viewPkgsButton, BorderLayout.EAST);
			...
			try
			{
				...
				if (menuClicked.getText().compareTo("Overview") == 0)
				{
					JFrame explainWindow =
						new JFrame("Overview for View Packages Sample");
					File explainFile = new File("Java_ViewPackagesUI_Explain.html");
	...
	private class allButtonsHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if (buttonPressed == viewPkgsButton)
			{
				ViewPackages packages = new ViewPackages();
				output = packages.viewPackages(connect);
			}
	...
	public static void main(String args[])
	{
		...
		ViewPackagesUI frame = new ViewPackagesUI("IBM Cognos Sample", connection);
