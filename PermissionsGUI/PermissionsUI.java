...
// display a window.
public class PermissionsUI extends JFrame
{
	...
	public PermissionsUI(String title, CRNConnect connection)
	{
	...
	private void addComponents()
	{
		...
		permissionButton = new JButton("Set Permissions");
		permissionButton.addActionListener(new allButtonsHandler());

		cmURLPanel.add(new JLabel("Server URL:"), BorderLayout.WEST);
		cmURLPanel.add(cmURL, BorderLayout.CENTER);
		cmURLPanel.add(permissionButton, BorderLayout.EAST);
		...
		panel.add(cmURLPanel, BorderLayout.NORTH);
			...
			try
			{
				...
				if (menuClicked.getText().compareTo("Overview") == 0)
				{
					JFrame explainWindow =
						new JFrame("Overview for Permissions Sample");
					File explainFile = new File("Java_PermissionsGUI_Explain.html");
	...
	private class allButtonsHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if (buttonPressed == permissionButton)
			{
				Permissions permissions = new Permissions();
				output = permissions.modifyPermissions(connect);
			}
	...
	public static void main(String args[])
	{
		...
		PermissionsUI frame = new PermissionsUI("IBM Cognos Sample", connection);
