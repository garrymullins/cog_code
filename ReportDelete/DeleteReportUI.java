...
// display a window.
public class DeleteReportUI extends JFrame
{
	...
	public DeleteReportUI(String title, CRNConnect connection)
	{
	...
	private JPanel createMainButtonPanel()
	{
		...
		deleteReportButton.addActionListener(new allButtonsHandler());
		buttonPanel.add(deleteReportButton, BorderLayout.EAST);
			...
			try
			{
				...
				if (menuClicked.getText().compareTo("Overview") == 0)
				{
					JFrame explainWindow =
						new JFrame("Overview for Delete Report Sample");
					File explainFile = new File("Java_DeleteReportUI_Explain.html");
	...
	private class allButtonsHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if (buttonPressed == deleteReportButton)
			{
					...
					// Refresh the report list when the report is deleted.
					repSelectOption.removeItem(selectedReport);
					repSelectOption.setSelectedItem(null);

					//Display a list of remaining reports
	...
	public static void main(String args[])
	{
		...
		DeleteReportUI frame = new DeleteReportUI("IBM Cognos Sample", connection);
