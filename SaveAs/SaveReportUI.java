...
// display a window.
public class SaveReportUI extends JFrame
{
	...
	private JComboBox saveCommand;
	private JComboBox repSelectOption;

	private static Logon sessionLogon;

	private static final String SAVE_REP_OUTPUT = "Save Report Output";
	private static final String SAVE_REP_VIEW_AS = "Save Report As";

	private static BaseClassWrapper selectedReport = null;
	private static boolean saveAs = false;

	// This is the constructor.
	public SaveReportUI(String title, CRNConnect connection)
	{
	...
	private void addComponents()
	{
		...
		String saveOption[] = { SAVE_REP_OUTPUT, SAVE_REP_VIEW_AS };
		saveCommand = new JComboBox(saveOption);
		saveCommand.setSelectedItem(null);
		saveCommand.addActionListener(new SaveCommandHandler());
		buttonPanel.add(saveCommand, BorderLayout.WEST);
			...
			try
			{
				...
				if (menuClicked.getText().compareTo("Overview") == 0)
				{
					JFrame explainWindow =
						new JFrame("Overview for Save Report Sample");
					File explainFile = new File("Java_SaveReportUI_Explain.html");
	...
	private class allButtonsHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if (buttonPressed == runOptionButton)
			{
				...
				{
					output =
						saveReport.saveReport(connect, selectedReport, saveAs);
				}
				catch (Exception ex)
				{
	...
	private class SaveCommandHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent repTypeSelectedEvent)
		{
			String chosenType = (String)saveCommand.getSelectedItem();
			if (chosenType == SAVE_REP_OUTPUT)
			{
				saveAs = false;
			}
			else if (chosenType == SAVE_REP_VIEW_AS)
			{
				saveAs = true;
			}
			else
			{
				//error, force SAVE_REP_VIEW_AS (default) ??
				saveAs = true;
			}
		}
	}
	...
	public static void main(String args[])
	{
		...
		SaveReportUI frame = new SaveReportUI("IBM Cognos Sample", connection);
