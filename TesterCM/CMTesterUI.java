...
// display a window.
public class CMTesterUI extends JFrame
{
	...
	private JButton cmInfoButton;

	private static Logon sessionLogon;

	// This is the constructor.
	public CMTesterUI(String title, CRNConnect connection)
	{
	...
	private void addComponents()
	{
		...
		cmInfoButton = new JButton("Content Manager Info");
		cmInfoButton.addActionListener(new allButtonsHandler());
		JPanel buttonPanel = new JPanel();
		buttonPanel.add(cmInfoButton, BorderLayout.EAST);
			...
			try
			{
				...
				if (menuClicked.getText().compareTo("Overview") == 0)
				{
					JFrame explainWindow =
						new JFrame("Overview for CM Tester Sample");
					File explainFile = new File("Java_CMTesterUI_Explain.html");
	...
	private class allButtonsHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if (buttonPressed == cmInfoButton)
			{
				CMTester sampleCMTester = new CMTester();
				output += sampleCMTester.contentMgrTester(connect);
			}
	...
	public static void main(String args[])
	{
		...
		CMTesterUI frame = new CMTesterUI("IBM Cognos Sample", connection);
