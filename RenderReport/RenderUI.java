...
public class RenderUI extends JFrame
{
	...
	public RenderUI(String title, CRNConnect connection)
	{
	...
	private JPanel createMainButtonPanel()
	{
		...
		renderButton.addActionListener(new allButtonsHandler());
		buttonPanel.add(renderButton, BorderLayout.EAST);
			...
			try
			{
				...
				if (menuClicked.getText() == "About")
				{
					JOptionPane.showMessageDialog(
						...
						"IBM Cognos Sample Application\n\n"
							+ "Version 1.0.0\n"
							+ "This application demonstrates the IBM Cognos SDK",
						"About the Samples",
				...
				if (menuClicked.getText().compareTo("Overview") == 0)
				{
					JFrame explainWindow =
						new JFrame("Overview for RenderUI");
					File explainFile = new File("Java_RenderUI_Explain.html");
	...
	private class allButtonsHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if (buttonPressed == renderButton)
			{
				Render reportRenderer = new Render();

				try
				{

					output = reportRenderer.renderReport(
						connect,
						selectedReport,
						null);
				}
	...
	public static void main(String args[])
	{
		...
		RenderUI frame = new RenderUI("IBM Cognos Sample", connection);
